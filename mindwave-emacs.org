...
#+begin_src emacs-lisp
  (defun mw-snapshot-eeg (file)
    "Copy the EEG ring into a file, and save it to a buffer."
    (interactive)
    (let ((snap (mw-snapshot/get-eeg-csv)))
      (with-temp-file (expand-file-name (read-file-name "File to save EEG state to?")) 
        (insert snap))))
  
  (defun mw-snapshot/get-eeg-csv ()
    "Return the snapshotted EEG as a string of CSV"
    (let ((eeg-ring (reverse (ring-elements mindwave-eeg-ring))))
      (apply 'concat (mapcar #'(lambda (e) (concat (number-to-string e) 
                                                   ",")) 
                             eeg-ring))))
