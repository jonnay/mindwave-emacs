...
#+begin_src emacs-lisp :tangle yes
  ...
  The connection type is dependent on the `mindwvave-connection-type' variable."
    (if mindwave-connect-with-serial
        (mindwave-get-serial-buffer)
      (mindwave-get-thinkgear-buffer)))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
      ...
      (while (< pos end)
        (if (and (= #xAA
            ...
            (let ((plen (aref output (+ 2 pos))))
              ...
              (when (not (= (aref output (+ 3 plen pos))
                            (mindwave/checksum-bytestream (substring output 
                                                                     (+ pos 3) 
                                                                     (+ plen pos 3)))))
                (error "Mindwave: Checksum doesn't match. (Expected %s got %s)"
                       (aref output (+ plen 3 pos))
                       (mindwave/checksum-bytestream (substring output
                                                                (+ pos 3) 
                                                                (+ plen pos 3)))))
              (setq pos (+ 3 pos))
              (while (< pos (+ plen 3))
                (if (> #x80 (aref output pos))
                  ...
                  (progn 
                    (case (aref output pos)
                      (#x80 (setq raw (mindwave/2byte-word-to-int (aref output (+ 2 pos))
                                                                    (aref output (+ 3 pos)))))
                      (#x83 (setq eegPower
                    ...
                    (setq pos (+ pos 
                                 (aref output (+ pos 1)) 
                                 2))))))
          (error "No proper sync packets"))
        (add-to-list 'out-packets (append packet 
                                          (if (not (null raw))
                                              (cons (cons 'rawEeg raw)
                                                    '()))
                                          (if (not (null eSense))
  ...
  (ert-deftest mindwave-serial/filter-test ()
    ...
    (should-error (mindwave-serial-filter-function nil "xx"))
    (let ((packet (concat (list #xAA #xAA #x20 #x02 #x00 #x83 #x18 #x00 #x00 #x94 #x00 #x00 #x42 #x00 #x00 #x0B #x00 #x00 #x64 #x00 #x00 #x4D #x00 #x00 #x3D #x00 #x00 #x07 #x00 #x00 #x05 #x04 #x0D #x05 #x3D #x34))))
      (should-not (null (mindwave-serial-filter-function nil packet)))
      
      (should-equal (mindwave-serial/filter-function nil packet)
                    '(((poorSignalLevel . 0)
                       (eSense . ((attention . 13) 
                                  (meditation . 61)))
                       (eegPower . ((delta . #x94)
  ...
  (defmacro mindwave-serial/make-eeg-list (eegName b1)
    (let ((b2 (+ 1 b1))
          (b3 (+ 2 b1)))
      `(cons ,eegName (mindwave/3byte-uword-to-int (aref output (+ ,b1 pos))
                                                   (aref output (+ ,b2 pos))
                                                   (aref output (+ ,b3 pos))))))
  ...
  (ert-deftest mindwave-serial/checksum-test ()
    ...
    (should (= (mindwave/checksum-bytestream (concat (list 0))) 255))
    (should (= (mindwave/checksum-bytestream (concat (list 255))) 0))
    (should (= (mindwave/checksum-bytestream (concat (list 255 1))) 255))
    (should (= (mindwave/checksum-bytestream (concat (list 255 1 255 1 255 1))) 254))
    (should (= (mindwave/checksum-bytestream (concat (list 0 1))) 254))
    (should (= (mindwave/checksum-bytestream (concat (list #x02 #x00 #x83 #x18 #x00 #x00 #x94 #x00 #x00 #x42 #x00 #x00 #x0B #x00 #x00 #x64 #x00 #x00 #x4D #x00 #x00 #x3D #x00 #x00 #x07 #x00 #x00 #x05 #x04 #x0D #x05 #x3D))) #x34)))
